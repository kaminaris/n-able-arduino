...
typedef struct
{
	...
	volatile uint8_t  VTFIDR[4];        // 50H (IDCFGR on ch582)
	uint8_t       RESERVED0[0x0C];  // 54H
	volatile uint32_t VTFADDR[4];       // 60H (FIADDRR on ch582)
...
// enter idle state
static  inline void LowPowerIdle(uint32_t cyc)
{
	RTCTrigger(cyc);
...
static  inline void LowPowerSleep(uint32_t cyc, uint16_t power_plan) 
{
...
static  inline void LowPower(uint32_t time, uint16_t power_plan) 
{
	if( time > 500){
...
static  inline void jump_isprom()
{
