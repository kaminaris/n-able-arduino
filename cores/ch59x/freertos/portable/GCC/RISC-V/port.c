...
static inline void SetVTFIRQ(uint32_t addr, IRQn_Type IRQn, uint8_t num, uint8_t NewState){
	if(num > 1)  return ;
...
{
    SetVTFIRQ((uint32_t)SW_Handler, Software_IRQn, 0, 1);
    SetVTFIRQ((uint32_t)SysTick_Handler, SysTick_IRQn, 1, 1);

    /* set software is lowest priority */
    PFIC_SetPriority(Software_IRQn, 0xf0);
    PFIC_EnableIRQ(Software_IRQn);
...
// __INTERRUPT
// __attribute__((section(VECTOR_HANDLER_SECTION)))
__attribute__((section(".highcode")))
__attribute__((interrupt("WCH-Interrupt-fast")))
