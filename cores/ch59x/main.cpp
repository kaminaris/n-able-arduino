...
int main(void) {
	...
	_loopTaskHandle = xTaskCreateStatic(
		loopTask, "mlt", MAIN_TASK_STACK_SIZE,
		NULL, 1, _mainStack, &_mainTaskBuffer
	);

	// volatile int taskCreated = (result == pdPASS) ? 1 : 0; // Inspect this
	// if (result != pdPASS) {
	// 	// Task creation failed - halt here
	// 	while (1);
	// }
