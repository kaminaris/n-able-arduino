...
FRAMEWORK_DIR = platform.get_package_dir("framework-n-able-arduino-riscv")
...
startupFilename = join("cores", "ch59x", "freertos")
machine_flags = [
	"-march=%s" % board.get("build.march"),
	"-mabi=%s" % board.get("build.mabi"),
	]
freertos_subseries = join("cores", "ch59x", "freertos")
env.Append(
	...
	CPPPATH=[
		join(FRAMEWORK_DIR, "cores", "ch59x"),
		...
		join(FRAMEWORK_DIR, "cores", "ch59x", "api", "deprecated"),
		join(FRAMEWORK_DIR, "cores", "ch59x", "api", "deprecated-avr-comp"),
		join(FRAMEWORK_DIR, "cores", "ch59x", "wch"),
